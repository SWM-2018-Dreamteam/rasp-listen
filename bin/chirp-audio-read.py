...
from chirp import ChirpConnect, CallbackSet, CHIRP_CONNECT_BUFFER_SIZE
...
def main(args):
    ...
    for f in range(0, len(data), CHIRP_CONNECT_BUFFER_SIZE):
        if w.getsampwidth() == 2:
            sdk.process_shorts_input(data[f: f + CHIRP_CONNECT_BUFFER_SIZE])
        elif w.getsampwidth() == 4:
            sdk.process_input(data[f: f + CHIRP_CONNECT_BUFFER_SIZE])

    # C SDK currently doesn't flush after the final frame
    sdk.process_input('\x00' * CHIRP_CONNECT_BUFFER_SIZE)
